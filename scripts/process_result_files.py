...
for file_path in walk_result_file_paths(root_path="results"):
    ...
    each_json_converted = {
        "hash_code": convert_string2unique_number(
            f"{each_json['result_group']}-"
        ...
        "cost": each_json["meta"]["monthly_cost"]
        / (100 * each_json["search_results"]["rps"]),
        "monthly_cost": each_json["meta"]["monthly_cost"],
        # index parameter
        "index_create_parameter": each_json["index_create_parameter"],
        # search parameter
        "index_search_params": each_json["index_search_parameter"].get(
            "params", each_json["index_search_parameter"].get("vectorIndexConfig", "")
        ),
